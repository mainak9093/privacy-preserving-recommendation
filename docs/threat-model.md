...
**Status:** first draft, 2026-09-10. Covers **S1 only** (private serving and delivery). Private
...
| **A6** | A network observer | Message sizes and timing | Sees public parameters and timing only. All message sizes are input-independent by construction. **But see §5: this is a design property, not something we have demonstrated on a wire.** |
...
- **Nothing has been demonstrated on a wire.** The three servers are three sets of shares held and
  combined in one process; `src/net/` is empty and networking is deferred to Phase 3. A6's
  guarantee follows from the construction — every message size is input-independent, and every
  DPF coefficient is a pseudorandom share rather than a mostly-zero vector — but the
  channel-boundary transcript and the distinguisher experiment that would *demonstrate* it have
  not been run. This is the largest open gap in the S1 security story and we name it rather than
  let the architecture's prose imply otherwise.
...
| — | Channel transcript + distinguisher experiment (A6) | Phase 3 |
...
`bench/bench_baseline.cpp`, raw rows in `bench/results/leakage_attack.jsonl` and
`bench/results/bench_baseline.jsonl`, figures regenerated by `mingw32-make figures`.*
